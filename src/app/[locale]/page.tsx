...
export default async function HomePage({ params }: PageProps) {
  ...
  const trendingTopics = latestArticles
    .slice(0, 5)
    .map((a) => ({ title: a.title, url: `/${locale}/${a.category?.slug}/${a.slug}` }));
    ...
    <div className="min-h-screen">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-4">
            {sidebarArticles.map((article) => (
              <ArticleCard key={article.id} article={article} variant="sidebar" locale={locale} />
            ))}
          </div>
        </div>
      ...
      <section className="bg-section-dark py-10">
        <div className="max-w-255 mx-auto px-4">
          <SectionTitle title={dict.home.mustRead} light />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
            {mustReadArticles.map((article) => (
              <ArticleCard key={article.id} article={article} variant="dark" locale={locale} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="max-w-255 mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-10">
          ...
          <div>
            ...
            {recapArticles[0] && (
              <ArticleCard article={recapArticles[0]} variant="recap-first" locale={locale} />
            )}
            {/* Articles texte uniquement */}
            <div className="space-y-0">
              {recapArticles.slice(1, 7).map((article) => (
                <ArticleCard key={article.id} article={article} variant="recap-text" locale={locale} />
              ))}
            </div>
            {/* Bloc réseaux sociaux */}
            <div className="mt-6 border-2 border-brand rounded-lg p-4 text-center">
              <p className="text-sm font-bold text-dark mb-3">{locale === "en" ? "Follow NoMask on" : "Retrouvez NoMask sur"}</p>
          ...
          <div>
            <SectionTitle title={dict.home.latestNews} />
            {/* 2 premiers articles */}
            <div className="mt-4 space-y-0">
              {lastNewsArticles.slice(0, 2).map((article) => (
                <ArticleCard key={article.id} article={article} variant="actus-row" locale={locale} />
              ))}
            </div>
            ...
            <div className="relative mt-4 mb-2 pl-4 pt-4 border-l-[3px] border-brand border-t-[3px]">
              <h3 className="text-xl font-black text-brand italic leading-tight">
                {locale === "en" ? "What you shouldn't have missed" : "Ce qu'il ne fallait pas manquer"}
              </h3>
            </div>
            {/* 4 articles suivants */}
            <div className="space-y-0">
        ...
        <div className="max-w-255 mx-auto px-4 relative">
          ...
          <div className="flex items-center mb-10 w-full relative z-20">
            <div className="flex items-center pr-4 bg-white">
              ...
              <h2 className="text-[26px] font-black tracking-tight text-gray-900">{dict.home.videos}</h2>
            </div>
            ...
            <div className="pl-4 bg-white">
               <a href="#" className="flex items-center gap-1 text-[13px] font-bold text-brand hover:text-brand-dark transition-colors uppercase tracking-wider relative group">
                 <span className="pb-0.5 border-b-[1.5px] border-brand group-hover:border-brand-dark">{locale === "en" ? "See all videos" : "Voir toutes les vidéos"}</span>
               ...
               <a href={ytVideos.main?.youtube_url ?? "#"} target="_blank" rel="noopener noreferrer" className="w-full relative block">
                 ...
                 <div className="mt-5 px-1 mb-8 max-w-4xl">
                   <div className="flex items-center gap-1.5 text-gray-400 text-[11px] font-bold uppercase mb-2">
                     ...
                     <span>{ytVideos.main?.source_name ?? (locale === "en" ? "Video" : "Vidéo")}</span>
                   </div>
          ...
          <div className="flex items-center mb-10 w-full relative z-20 overflow-visible">
            ...
            <div className="pl-4 bg-white/80 backdrop-blur-sm relative z-10">
               <a href="#" className="flex items-center gap-1 text-[13px] font-bold text-brand hover:text-brand-dark transition-colors uppercase tracking-wider group relative">
                 <span className="pb-0.5 border-b-[1.5px] border-brand group-hover:border-brand-dark">{locale === "en" ? "See all articles" : "Voir tous les articles"}</span>
                 <span className="text-xl font-light leading-none -mt-0.5">+</span>
               </a>
            </div>
          </div>
                ...
                <div key={article.id} className="relative w-full h-[160px] md:h-[176px] group overflow-hidden bg-gray-200 flex items-stretch">
                  ...
                  <div className={`absolute top-0 bottom-0 ${isLeftText ? 'left-0 w-[65%]' : 'right-0 w-[65%]'} bg-[#303440]/[0.94] z-10 flex flex-col justify-center px-8 md:px-12`}>
                     ...
                     <div className="flex items-start gap-3">
                        ...
                        <a href={`/${locale}/${article.category?.slug}/${article.slug}`} className="hover:text-brand transition-colors">
                     ...
                     <div className="flex items-center gap-3 mt-4">
                        ...
                        <div className="flex items-center flex-wrap gap-x-2 text-[13px]">
                          ...
                          <span className="text-gray-300 font-medium">
                            {locale === "en" ? "Published" : "Publié le"} {article.published_at ? new Date(article.published_at).toLocaleDateString(locale === "en" ? 'en-US' : 'fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' }).replace(/\//g, '.') : '—'}
                          </span>
                        </div>
                     </div>
                  </div>
      ...
      <section className="bg-[#f7f8fa] py-10 border-t border-gray-200">
        <div className="max-w-255 mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <ThemeColumn
              title={dict.home.thematic.sport}
            ...
            />
            <ThemeColumn
              title={dict.home.thematic.economie}
      ...
      {/* ======= TOUTE L'ACTUALITÉ ======= */}
      <section className="max-w-255 mx-auto px-4 py-10">
        <h2 className="text-[30px] font-black tracking-tight text-gray-900 mb-8">{dict.home.allNews}</h2>
          ...
          <div className="flex flex-col">
            {allNewsArticles.slice(0, 5).map((article, idx) => (
              <div key={article.id}>
                ...
                <Link href={`/${locale}/${article.category?.slug}/${article.slug}`} className="flex gap-4 group py-3 border-b border-gray-100 last:border-0">
        ...
        <div className="max-w-255 mx-auto px-4">
          <div className="flex items-start justify-between mb-8">
            <h2 className="text-[26px] font-black text-white tracking-tight">
              <span className="text-[#8b8cf8]">🏷️</span> {locale === "en" ? "Our tested picks" : "Nos sélections testées"}
            </h2>
            <Link
              href={`/${locale}/tech`}
            ...
            >
              {locale === "en" ? "See all guides" : "Voir tous les guides"} <span className="inline-block ml-0.5">+</span>
            </Link>
          </div>
